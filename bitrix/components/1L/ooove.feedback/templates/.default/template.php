...
<div class="bx_mfeedback bx_<?=$arResult["THEME"]?>">
	...
	<form action="<?=POST_FORM_ACTION_URI?>" method="POST">
		...
		<input type="text" placeholder="Имя*" name="user_name" value="<?=$arResult["AUTHOR_NAME"]?>"/><br/>

		<!--<strong><?=GetMessage("MFT_PHONE")?><?if(empty($arParams["REQUIRED_FIELDS"]) || in_array("EMAIL", $arParams["REQUIRED_FIELDS"])):?><span class="mf-req">*</span><?endif?></strong></br>-->
		<input type="text" name="PERSONAL_PHONE" placeholder="Телефон*" value="<?=$arResult["PERSONAL_PHONE"]?>"/><br/>
		...
		<input type="text" name="user_email" placeholder="емэйл*" value="<?=$arResult["AUTHOR_EMAIL"]?>"/><br/>
		...
		<textarea name="MESSAGE" placeholder="Введите текст Вашего сообщения" rows="5" cols="40"><?=$arResult["MESSAGE"]?></textarea><br/>
		...
		<input type="submit" name="submit" value="<?=GetMessage("MFT_SUBMIT")?>" class="btn btn-primary btn-xs">
	</form>
</div>