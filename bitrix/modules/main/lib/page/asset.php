...
class Asset
{
	 ...
	 * @param int $mode Set current composit mode.
	 */
	...
	/**
	 * @param $value boolean Use xhtml html style
	 */
	...
	/**
	 * @param $value integer Count of css files showed inline fore ie
	 * @deprecated
	 */
	...
	/**
	 * @return boolean Optimization off or on for js
	 */
	...
	private function optimizeAsset($files = array(), $unique = false, $prefix = 'default', $setName = '', $type = 'css', $data = '')
	{
		...
		if(!$writeResult)
		{
			foreach($files as $file)
			{
				$currentFileList[] = $file['FULL_PATH'];
				if ($type == 'css')
				{
