...
<div class="api-feedback auth-wind" style="<?= $arParams['FORM_STYLE'] ?>">
	<?= $arResult['FORM_TITLE'] ?>
	...
	<form action="<?= $APPLICATION->GetCurPage() ?>" method="POST" enctype="multipart/form-data" name="api_feedback_form" id="<?= $arParams['UNIQUE_FORM_ID']; ?>">
		...
		<? if($arParams['USE_HIDDEN_PROTECTION']): ?>
			<input type="text" name="hidden_protection" class="hidden_protection" value="<?=$_REQUEST['hidden_protection'];?>" style="display:none" />
		<? endif; ?>
		...
		if(count($arParams['DISPLAY_FIELDS']) > 0)
		{
			foreach($arParams['DISPLAY_FIELDS'] as $FIELD)
			{
				...
				{?>
				<div class="form-input <?=$tpl_class_name;?>_<?= ToLower($FIELD) ?>" style="<?= $arParams['FORM_STYLE_DIV'] ?>">
			...
			<?foreach($arParams['CUSTOM_FIELDS'] as $fk => $FIELD)
			{
				...
				switch($arFields[1])
				{
					...
					case "input":
                        ...
                        {
	                        $values = array();//»звлекаемый массив
	                        foreach($arFields as $arField)
	                        {
		                        if(substr($arField,0,6)=="values")
		                        {
		...
		<div class="api-submit">
			<input type="submit" name="submit" class="btn red" style="<?= $arParams['FORM_STYLE_SUBMIT'] ?>" value="<?= $arResult['FORM_SUBMIT_VALUE'] ?>">
		</div>
	</form>
</div>