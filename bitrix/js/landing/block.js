;(function() {
	...
	BX.Landing.Block.prototype = {
		...
		{
			var selector = "a:not([class*='landing-ui']), .btn:not([class*='landing-ui']), button:not([class*='landing-ui']), input[type=\"submit\"]:not([class*='landing-ui']), input[type=\"button\"]:not([class*='landing-ui'])";
				...
				if (labelNode)
				{
					...
					if (labelNode instanceof BX.Landing.Block.Node.Text)
					{
						currentLabel = create("span", {
							...
							html: create("div", {html: labelNode.getValue()}).innerText
						});
						children.push(currentLabel);

						onCustomEvent(labelNode.getField(), "BX.Landing.UI.Field:change", function(value) {
							currentLabel.innerHTML = create("div", {html: value}).innerText;
						});
